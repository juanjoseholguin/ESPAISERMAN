...
export default function renderActiveGame({ roomCode } = {}) {
  ...
  // Recuperar preguntas de la sala
  ...
  // Convertir preguntas de Supabase al formato que espera la UI
  const formattedQuestions = questions.map(q => {
    // Supabase usa: question, answer (json array), correct_answer
    let answers = [];
    if (q.answer && Array.isArray(q.answer)) {
      answers = q.answer;
    } else {
      // Fallback si answer no está en formato array
      answers = [
        q.correct_answer,
    ...
    <div class="screen active">
      <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px;">
        <div style="display:flex; align-items:center; gap:8px; background:#FFE28A; padding:8px 16px; border-radius:16px;">
          ...
          <span style="font-weight:800; color:#1e3a8a; font-size:16px;">${window.memoryState.currentUserCoins || 1000}</span>
        </div>
      ...
      <div style="background:rgba(255,255,255,0.95); border-radius:16px; padding:20px; margin:0 16px;">
        ...
        <div style="display:flex; flex-direction:column; gap:10px;">
          ${question.options.map((opt, idx) => `
            <button class="answer-btn" data-answer="${opt}" style="background:${['#E34C43', '#11A36B', '#FFB347', '#8FA6E0'][idx]}; padding:12px; border:none; border-radius:12px; text-align:left; color:white; font-size:16px;">
