...
function initMap() {
  ...
  const icesiPoints = [
    ...
    { lat: 3.3400, lng: -76.5300, name: "Parqueadero", questionId: 6 },
    { lat: 3.3420, lng: -76.5300, name: "Cancha Deportiva", questionId: 7 }
  ];
...
async function loadQuestion(questionId) {
  try {
    ...
    questionAnswers.innerHTML = answers.map((answer, index) => `
...
function startQRScan() {
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    const modal = document.createElement('div');
    modal.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,.9); display:flex; align-items:center; justify-content:center; z-index:1000;';
    modal.innerHTML = `
