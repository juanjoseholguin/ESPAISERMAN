      ...
      document.getElementById('create-room-btn').addEventListener('click', async () => {
        ...
        try {
          ...
          if (!Array.isArray(questions) || questions.length === 0) {
            alert('No hay preguntas disponibles para esta categoría en Supabase. Asegúrate de que la tabla questions tenga datos.');
            return;
          }
          ...
          // Limitar a 5 preguntas para evitar payload too large
...
function initMap(category) {
  ...
  const icesiCoords = [3.3440, -76.5329];
  ...
  const points = [
    { name: 'Edificio A', coords: [3.3435, -76.5330] },
    { name: 'Biblioteca', coords: [3.3438, -76.5332] },
    { name: 'Cafetería', coords: [3.3442, -76.5328] },
    { name: 'Auditorio', coords: [3.3439, -76.5325] },
    { name: 'Laboratorios', coords: [3.3445, -76.5330] }
  ];

  points.forEach((point, idx) => {
    const icon = L.divIcon({
      className: 'custom-question-marker',
      html: `<div style="background-color: #FFE28A; width: 36px; height: 36px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 3px solid #1e3a8a; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"><span style="color: #1e3a8a; font-weight: bold; font-size: 16px;">${idx + 1}</span></div>`,
      iconSize: [36, 36],
      iconAnchor: [18, 18]
    });
